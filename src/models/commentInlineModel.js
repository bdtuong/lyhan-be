...
const validateBeforeCreate = async (data) => {
    return await COMMENT_COLLECTIONINLINE_SCHEMA.validateAsync(data, {abortEarly: false})
}
...
const createComment = async (data) => {
    try {
        ...
        const createdComment = await GET_DB().collection(COMMENT_COLLECTIONINLINE_NAME).insertOne(validData)
...
const findOneById = async (id) => {
    try {
        return await GET_DB().collection(COMMENT_COLLECTIONINLINE_NAME).findOne({_id: new ObjectId(id)})
    } catch (error) {
        throw new Error(error)
    }
}
...
const getDetails = async (id) => {
    try {
        return await GET_DB().collection(COMMENT_COLLECTIONINLINE_NAME).aggregate([
            { $match: { 
                _id: new ObjectId(id),
                _destroy: false
            } },
